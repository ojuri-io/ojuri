...
function TransactionsList({ toast, nav, queue: _queue }) {
  ...
  // (Previously held a synthetic-data fallback so the page looked alive
  // when the API was unreachable. Removed 2026-05-16 — empty state now
  // wins, no fake rows.)
  ...
  const counts = useMemo(() => {
    // Only the active "ALL" count is precise (server `total`);
    // arbitrary segments would each need their own fetch, so a dot
    // keeps the UI quiet instead of pretending to know.
    if (rows && rows.length > 0) {
      return { ALL: total || 0, ACCEPT: '·', DECLINE: '·', REVIEW: '·', OVERRIDDEN: '·' };
    }
    return { ALL: total || 0, ACCEPT: 0, DECLINE: 0, REVIEW: 0, OVERRIDDEN: 0 };
  }, [rows, total]);
