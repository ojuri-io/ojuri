 ...
 *   - An `admin` user with bcrypt-hashed password `admin@fraudit`,
 *     assigned to SUPER_ADMIN. The plaintext password is exposed in
 *     this migration on purpose — first-run experience matters more
 *     than secrecy for a self-hosted demo platform. Adopters should
 *     change it via the `PATCH /v1/admin/users/:id` endpoint.
 */
export async function up(knex: Knex): Promise<void> {
  ...
  const passwordHash = await bcrypt.hash("admin@fraudit", 10);
