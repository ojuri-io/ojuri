  ...
  window = async (
    ...
    return res.send(
      SuccessResponse("Window stats", {
        ...
        total,
        rates: {
          decline: total > 0 ? counts.DECLINE / total : 0,
          accept: total > 0 ? counts.ACCEPT / total : 0,
          review: total > 0 ? counts.REVIEW / total : 0,
          override: total > 0 ? overrides / total : 0,
        },
        decisionsPerSec,
        declineRate: total > 0 ? counts.DECLINE / total : 0,
  ...
  today = async (
    ...
    return res.send(
      SuccessResponse("Today's stats", {
        ...
        counts,
        rates: {
          acceptRate: total ? counts.ACCEPT / total : 0,
          declineRate: total ? counts.DECLINE / total : 0,
